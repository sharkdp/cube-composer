...
describe('transformationSteps', function(){
    ...
    it('returns a list of the intermediate results, starting with the initial array', function(){
        var f1 = map2D(clone);
        var f2 = filter2D(R.eq(1));

        var second = cleanup(f1(first));
        var third = cleanup(f2(second));

        transformationSteps(first, [f1, f2]).should.eql([first, second, third]);
    });
});
